...
try:
    from src.rich_fusion_seq2seq import RichFusionEncoderDecoder, load_compatible_fusion_weights
except ImportError:
    from rich_fusion_seq2seq import RichFusionEncoderDecoder, load_compatible_fusion_weights
...
def _as_float_tensor(item: dict, key: str) -> torch.Tensor:
    value = item[key]
...
def _pad_sequence_tensors(batch: List[dict], key: str) -> tuple[torch.Tensor, torch.Tensor]:
    sequences = [_as_float_tensor(item, key) for item in batch]
...
def build_collate_fn(tokenizer: AutoTokenizer, max_target_len: int, emotion_to_id: Dict[str, int]):
    ...
    def collate_fn(batch: List[dict]) -> Dict[str, torch.Tensor | List[str]]:
        ...
        labels = label_tokens["input_ids"].masked_fill(label_tokens["input_ids"] == pad_token_id, -100)

        face_tokens, face_mask = _pad_sequence_tensors(batch, "face_tokens")
        audio_tokens, audio_mask = _pad_sequence_tensors(batch, "audio_tokens")
        text_tokens, text_mask = _pad_sequence_tensors(batch, "text_tokens")
        ...
        return {
            ...
            "text_mask": text_mask,
            "face_emo": torch.stack([_as_float_tensor(item, "face_emo_logits") for item in batch], dim=0),
            "a_spk": torch.stack([_as_float_tensor(item, "audio_speaker") for item in batch], dim=0),
            "a_pros": torch.stack([_as_float_tensor(item, "audio_prosody") for item in batch], dim=0),
            "emo": torch.tensor(
...
def maybe_freeze_fusion_backbone(model: RichFusionEncoderDecoder, unfreeze_last_nlayers: int) -> None:
...
def build_optimizer(model: RichFusionEncoderDecoder, lr_fusion: float, lr_decoder: float) -> torch.optim.Optimizer:
    fusion_params = []
    decoder_params = []
    for name, param in model.named_parameters():
...
@torch.no_grad()
def evaluate(
    model: RichFusionEncoderDecoder,
    loader: DataLoader,
    device: str,
    w_emo: float,
    w_arousal: float,
...
def main() -> None:
    ...
    parser.add_argument("--pre_ckpt", type=str, required=True, help="Fusion checkpoint to warm-start from")
    parser.add_argument("--train_pt", type=str, required=True, help="Rich training dataset (.pt)")
    parser.add_argument("--val_pt", type=str, default="", help="Rich validation dataset (.pt). If omitted, split train_pt")
    parser.add_argument("--out_pt", type=str, default="out/fusion_seq2seq_rich/best.pt")
    ...
    parser.add_argument("--max_memory_tokens", type=int, default=320)
    ...
    train_data = torch.load(args.train_pt, map_location="cpu")
    train_data = [item for item in train_data if str(item.get("target_text", "")).strip()]
    if not train_data:
        raise RuntimeError("No samples with target_text were found in the rich dataset.")
    ...
    if args.val_pt:
        val_data = torch.load(args.val_pt, map_location="cpu")
    ...
    backbone_cfg = {
        ...
        "max_memory_tokens": cfg.get("max_memory_tokens", args.max_memory_tokens),
    }
    ...
    model = RichFusionEncoderDecoder(
        ...
        max_memory_tokens=backbone_cfg["max_memory_tokens"],
    ).to(device)
